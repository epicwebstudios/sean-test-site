	...
	<thead>
		<tr>
			<td colspan="200">
                ...
                <div class="r">
                	Filter by Category: <? field_select2( 'f_c', $categories, $_GET['f_c'], '', 'onchange="set_filter();"' ); ?>
                </div>
		...
		?>
            ...
            <tr <? if( $info['status'] == '0' ){ echo 'class="inactive"'; } ?>>
                ...
                <? // Column output start ?>
					...
					<td>
						<? echo $categories[$info['category']]; ?>
                    </td>
                
					<td>
						<? if( $info['link_type'] == '1' ){ ?>
                        	<a href="<? echo returnURL().'/uploads/resources/'.$info['filename']; ?>" target="_blank">
                            	<? echo $info['filename']; ?>
                            </a>
                        <? } else { ?>
                        	<a href="<? echo $info['url']; ?>" target="_blank">
                            	<? echo $info['url']; ?>
                            </a>
                        <? } ?>
                    </td>
		...
		?>
            ...
            <tr>
                <td colspan="200" align="center">
                    <? if( $_GET['s'] ){ ?>
                    	<? if( $_GET['f_c'] ){ ?>
                        	Your search for "<? echo $_GET['s']; ?>" under "<? echo $categories[$_GET['f_c']]; ?>" returned no <? echo $item_plural; ?>.
                        <? } else { ?>
                        	Your search for "<? echo $_GET['s']; ?>" returned no <? echo $item_plural; ?>.
                       	<? } ?>
                    <? } else { ?>
                    	<? if( $_GET['f_c'] ){ ?>
                        	There are currently no <? echo $item_plural; ?> under "<? echo $categories[$_GET['f_c']]; ?>" in the system.
                        <? } else { ?>
