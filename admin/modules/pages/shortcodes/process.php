	...
	if( (isset($_POST['edit_sub'])) || (isset($_POST['add_sub'])) ){
		...
		// -- Set values
			...
			$values = array(
				...
				'tag'		=> $_POST['tag'],
				'b_replace'	=> $_POST['b_replace'],
				'e_replace'	=> $_POST['e_replace'],
				'replace'	=> $_POST['replace'],
				'status'	=> $_POST['status'],
			);
			
			$values['tag'] 			= str_replace( '{', '', $values['tag'] );
			$values['tag'] 			= str_replace( '}', '', $values['tag'] );
			$values['b_replace'] 	= str_replace( '<?', '#?', $values['b_replace'] );
			$values['b_replace'] 	= str_replace( '?>', '?#', $values['b_replace'] );
			$values['e_replace'] 	= str_replace( '<?', '#?', $values['e_replace'] );
			$values['e_replace'] 	= str_replace( '?>', '?#', $values['e_replace'] );
			$values['replace'] 		= str_replace( '<?', '#?', $values['replace'] );
			$values['replace'] 		= str_replace( '?>', '?#', $values['replace'] );
