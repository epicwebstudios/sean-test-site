...
<html>
	...
	<head>
    	...
    	<script type="text/javascript">
			...
			function set_embed(){
				...
				if( video_id.indexOf('vimeo.com') >= 0 ){
					var explode = video_id.split(".com/");
				...
				if(video_id != ''){
					...
					if( information.options[information.selectedIndex].value == '1'){
						url_params += '&title=1&byline=1&portrait=1';
					} else {
						url_params += '&title=0&byline=0&portrait=0';
					}
					
					var value = '';
					value += '<div class="resp_video">';
					value += '<iframe src="https://player.vimeo.com/video/'+url_params+'" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>';
					value += '</div>';
	...
	<body>
		<p>Insert a Vimeo video directly into this page.</p>
        
        <input type="hidden" id="embed" value="">
        
        <div class="set">
        	<div class="label">Vimeo URL or ID:</div>
            <div class="field">
            	<input type="text" id="video_id" placeholder="Example: https://vimeo.com/6215179 or just 6215179" onkeyup="set_embed();">
            </div>
        </div>
