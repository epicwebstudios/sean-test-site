<?
	...
	/**
	 * mysql_*() to mysqli_*() compatibility definitions.
	 *
	 * Recreates deprecated / removed mysql_*() functions in newer versions of PHP.
	 *
	...
	if( !function_exists('mysql_connect') ){
		...
		function mysql_query( $query, $link_identifier = false ){
			...
			if( mysql_error() ){
				$bt 	= debug_backtrace();
				$bt 	= $bt[0];
				$log	= 'PHP Warning: mysql_query() error: "'.mysql_error().'" on '.$bt['file'].':'.$bt['line'];
				error_log( $log );
				return false;
			}
